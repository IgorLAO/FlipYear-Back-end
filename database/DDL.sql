CREATE DATABASE FLIPYEARDB;

USE FLIPYEARDB;

CREATE TABLE CLIENTES_TB(
	ID_CLIENTE			INT primary KEY auto_increment,
    ID_ENDERECO			INT  NOT NULL,
    NM_USUARIO			VARCHAR(100) NOT NULL,
    DS_TELEFONE			VARCHAR(100) NOT NULL,
    DS_CPF				VARCHAR(11)  NOT NULL,
    DS_EMAIL			VARCHAR(100) NOT NULL,
    DS_SENHA			VARCHAR(50) NOT NULL,
	DS_TIER				VARCHAR(50) DEFAULT ('NORMAL_USER') NOT NULL,
    foreign key (ID_ENDERECO) REFERENCES ENDERECOS_TB(ID_ENDERECO)
);

CREATE TABLE LOGIN_ALL_USERS_TB(
	ID_USER			INT primary KEY auto_increment,
    ID_ADM			INT,
	ID_CLIENTE		INT,
    FOREIGN KEY (ID_ADM)  REFERENCES ADMS_TB(ID_ADM),
    FOREIGN KEY (ID_CLIENTE) REFERENCES CLIENTES_TB(ID_CLIENTE)
);


CREATE TABLE ADMS_TB(
	ID_ADM			INT PRIMARY KEY auto_increment,
    NM_ADM			VARCHAR(100),
    DS_EMAIL		VARCHAR(100),
	DS_SENHA		VARCHAR(100),
    DS_TIER         VARCHAR(50) DEFAULT ('ADM')
);

CREATE TABLE ENDERECOS_TB(
	ID_ENDERECO			INT primary KEY auto_increment,
    DS_CEP				INT NOT NULL,
    NM_CIDADE			VARCHAR(100) NOT NULL,
    NM_RUA				VARCHAR(100) NOT NULL,
    DS_COMPLEMENTO		VARCHAR(100),
    NR_NUMERO			INT NOT NULL
);
						
-- ------------------------ --
-- ------------------------ --

CREATE TABLE PRODUTO_TB(
	ID_PRODUTO					INT primary KEY auto_increment,
	ID_CATEGORIA				INT NOT NULL,
	NM_PRODUTO					VARCHAR(100) NOT NULL,
    VL_PRECO					DECIMAL(21, 4) NOT NULL,
	VL_PRECO_PROMOCIONA			DECIMAL(21, 4) NOT NULL,
    BT_DESTAQUE					BOOLEAN NOT NULL,
    BT_PRMOCAO					BOOL NOT NULL,
    BT_DISPONIVEL				BOOL NOT NULL,
    QTD_ESTOQUE					INT NOT NULL,
    DS_DETALHES					varchar(500) NOT NULL,
    foreign key (ID_CATEGORIA) REFERENCES CATEGORIA_TB(ID_CATEGORIA)    
);


CREATE TABLE CATEGORIA_TB (
	ID_CATEGORIA				INT primary KEY auto_increment,
    NM_CATEGORIA				VARCHAR(100)
);

CREATE TABLE IMAGEM_PRODUTO_TB(
	ID_IMAGEM_PRODUTO			INT primary KEY auto_increment,
    ID_PRODUTO					INT,
    IMG_PRODUTO					VARCHAR(100),
    foreign key (ID_PRODUTO) REFERENCES	PRODUTO_TB(ID_PRODUTO)
);	







